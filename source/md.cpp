...
#ifdef WITH_SEGAVR
int M68K_CYCLES_PER_LINE = (MCLK_CYCLES_PER_LINE / 7);
int M68K_CYCLES_HBLANK = ((M68K_CYCLES_PER_LINE * 36) / 209);
int M68K_CYCLES_VDELAY = ((M68K_CYCLES_PER_LINE * 36) / 156);
int Z80_CYCLES_PER_LINE = (MCLK_CYCLES_PER_LINE / 15);
int Z80_CYCLES_HBLANK = ((Z80_CYCLES_PER_LINE * 36) / 209);
int Z80_CYCLES_VDELAY = ((Z80_CYCLES_PER_LINE * 36) / 156);
#endif
...
int md::reset()
{
	...
	M68K_CYCLES_PER_LINE = (MCLK_CYCLES_PER_LINE / m68kd);
	M68K_CYCLES_HBLANK = ((M68K_CYCLES_PER_LINE * 36) / 209);
	M68K_CYCLES_VDELAY = ((M68K_CYCLES_PER_LINE * 36) / 156);
	Z80_CYCLES_PER_LINE = (MCLK_CYCLES_PER_LINE / z80d);
	Z80_CYCLES_HBLANK = ((Z80_CYCLES_PER_LINE * 36) / 209);
	Z80_CYCLES_VDELAY = ((Z80_CYCLES_PER_LINE * 36) / 156);
#endif
	if (romCopy)
	{
		//restore base copy
	...
	memset(rom_bank_offsets, 0, sizeof(rom_bank_offsets));
	// Reset the VDP.
	vdp.reset();
	// Erase CPU states.
...
void gx_z80_writemem(unsigned int address, unsigned char data)
{
	md::md_gxz80->z80_write(address, data);
}

unsigned char gx_z80_readmem(unsigned int address)
{
	return md::md_gxz80->z80_read(address);
}

void gx_z80_writeport(unsigned int port, unsigned char data)
{
	md::md_gxz80->z80_port_write(port, data);
}

unsigned char gx_z80_readport(unsigned int port)
{
	return md::md_gxz80->z80_port_read(port);
}
#endif
...
#ifdef WITH_GXZ80
static int gxz80_irq_callback(int param)
{
	//return -1;
	//might as well, since it's hooked up for the other cores
	return md::md_gxz80->get_z80_irq_vector();
}
#endif
...
#ifdef WITH_GXZ80
	md_set_gxz80(1);
	gx_z80_init(NULL, gxz80_irq_callback);
	gx_z80_reset();
	z80_writemem = gx_z80_writemem;
	z80_readmem = gx_z80_readmem;
	z80_writeport = gx_z80_writeport;
	z80_readport = gx_z80_readport;
	md_set_gxz80(0);
#endif
#ifdef WITH_CZ80
...
bool md::init_sound()
{
	...
	if (dgen_ym_chipimpl == YM_CHIPIMPL_NUKEDOPN2)
	{
		...
		OPN2_Reset(&opn2);
		ym3438_sample[0] = ym3438_sample[1] = ym3438_sample_prev[0] = ym3438_sample_prev[1] = ym3438_cycles = ym3438_frame_ptr = ym3438_frame_cycles = 0;
		memset(ym3438_frame_buffer, 0, sizeof(ym3438_frame_buffer));
	}
	else
	{
#endif
...
#endif
#ifdef WITH_GXZ80
	md_gxz80_ref(0), md_gxz80_prev(0),
#endif
#ifdef WITH_SEGAVR
...
// in Asgard68000, so just in case...
  ...
  for (int i = 0; i < 16; ++i)
  {
	  z80_readmap[i] = mem + 0x10000 + (i << 10);
  }
  static uint8_t zeroZ80Read[0x400] = { 0 };
  for (int i = 16; i < 32; ++i)
  {
	  z80_readmap[i] = zeroZ80Read;
  }
  uint8_t *pZ80RamBank = ram + (z80_bank68k & 0x00FFFF);
  for (int i = 32; i < 64; ++i)
  {
	  z80_readmap[i] = pZ80RamBank + (i << 10);
  }
#endif
...
uint8_t md::region_guess()
{
	...
	//try checking for a new-style country code
	for (i = 0; (i != sizeof(this->cart_head.countries)); ++i)
	{
		switch (avail[i])
		{
