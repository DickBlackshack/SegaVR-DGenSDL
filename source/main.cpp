...
#ifdef _MSC_VER
#include "vcproj/DGenSDL/DGenSDL/resource.h"
...
void usleep(LONGLONG uSec)
{
	...
	HANDLE sleepTimer = CreateWaitableTimer(NULL, TRUE, NULL);
...
#ifdef _MSC_VER
int getopt_gnucantmakeathreadsafecommandlineparser_thatsrightisaidit(int &currentArg, char **ppOptArg, int argc, char *argv[], const char *pArgList)
{
	//don't know how these rules match up to getopt, the documentation would probably make me weep
	while (currentArg < argc)
	{
		char *pArg = argv[currentArg++];
		if (pArg[0] == '-' && pArg[1])
		{
			for (int listIndex = 0; pArgList[listIndex]; ++listIndex)
			{
				if (pArg[1] == pArgList[listIndex])
				{
					if (pArgList[listIndex] == ':')
					{
						*ppOptArg = argv[currentArg++];
					}
					return pArg[1];
				}
			}
	...
	//dumb hack, put it back on the filename (guess the normal implementation leaves "optind" on the current arg instead of going past)
	if (currentArg > 0)
	{
		--currentArg;
	}
...
int main(int argc, char *argv[])
{
  ...
  int optind = 0;
  char *optarg = NULL;
  while((c = getopt_gnucantmakeathreadsafecommandlineparser_thatsrightisaidit(optind, &optarg, argc, argv, temp)) != EOF)
#else
  ...
  {
	  HICON icon = LoadIcon(SDL_Instance, (LPCTSTR)IDI_ICON1);
	  if (icon)
	  {
		  SendMessage(SDL_Window, WM_SETICON, ICON_SMALL, (LPARAM)icon);
		  SendMessage(SDL_Window, WM_SETICON, ICON_BIG, (LPARAM)icon);
	  }
  }
#endif
...
#ifdef _MSC_VER
	rom = (optind > 0) ? argv[optind] : NULL;
#else
	rom = argv[optind];
#endif
	// Create the megadrive object.
	megad = new md(dgen_pal != 0, dgen_region);
...
next_rom:
	...
	while (!stop) {
		...
		stop |= (pd_handle_events(*megad) ^ 1);
#ifdef WITH_SEGAVR
		megad->segavr_apply_hmd_movement();
#endif
...
//will never be called, but ops are generated by default
extern "C" void m68040_fpu_op0(void)
{
}
extern "C" void m68040_fpu_op1(void)
{
}
