...
STATICINLINE uint16_t h2le16(uint16_t v)
{
...
STATICINLINE uint16_t h2be16(uint16_t v)
{
#ifdef WORDS_BIGENDIAN
	return v;
#else
...
STATICINLINE uint32_t h2le32(uint32_t v)
{
#ifdef WORDS_BIGENDIAN
...
STATICINLINE uint32_t h2be32(uint32_t v)
{
#ifdef WORDS_BIGENDIAN
	return v;
#else
...
STATICINLINE uint24_t *u24cpy(uint24_t *dst, const uint24_t *src)
{
...
STATICINLINE size_t strcommon(const char *s1, const char *s2)
{
	size_t i;
