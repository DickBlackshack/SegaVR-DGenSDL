...
{
	if (!screen.is_opengl || screen.swap_interval == ivl)
	{
		return;
	}

	if (SDL_SetSwapInterval(ivl))
	{
		screen.swap_interval = ivl;
	}
}
#endif
...
#ifdef _MSC_VER
static double pd_milliseconds()
{
	double r = 0.0;

	static LARGE_INTEGER initialCount = { 0 };
	static LARGE_INTEGER perfCountFreq = { 0 };

	if (!initialCount.QuadPart)
	{
	...
	{
		r = ((double)perfCount.QuadPart - initialCount.QuadPart) / ((double)perfCountFreq.QuadPart / 1000.0);
	}
	else
	{
		r = (double)GetTickCount();
	}
...
{
#ifdef _MSC_VER
	return (unsigned long)(pd_milliseconds() * 1000.0);
#else
	struct timeval tv;
...
retry:
	snprintf(name, sizeof(name), "%s-%s%06u.tga",
		 ((megad.romname[0] == '\0') ? "unknown" : megad.romname), pNameSuffix, n);
...
#ifdef WITH_GXZ80
		case 4:
			megad.z80_core = md::Z80_CORE_GXZ80;
			break;
#endif
		default:
...
#ifdef WITH_SEGAVR
	CTL_SEGAVR_HMD_PITCH_UP,
	CTL_SEGAVR_HMD_PITCH_DOWN,
...
#ifdef WITH_SEGAVR
static int ctl_hmd(struct ctl& ctl, md& megad)
{
	const float kInScale = 1.0f / 65536.0f;

	const bool isPitch = (ctl.type == CTL_SEGAVR_HMD_PITCH_UP || ctl.type == CTL_SEGAVR_HMD_PITCH_DOWN);
	const float moveAmount = (ctl.hasAxisAmount) ? kInScale * ctl.axisAmount * ((isPitch) ? dgen_segavr_hmd_pitch_scale : dgen_segavr_hmd_yaw_scale) :
	                                               kInScale * ((isPitch) ? dgen_segavr_hmd_pitch_const : dgen_segavr_hmd_yaw_const);

	switch (ctl.type)
	{
	case CTL_SEGAVR_HMD_PITCH_UP:
		megad.segavr_set_hmd_movement(0, moveAmount);
		break;
	case CTL_SEGAVR_HMD_PITCH_DOWN:
		megad.segavr_set_hmd_movement(0, -moveAmount);
		break;
	case CTL_SEGAVR_HMD_YAW_LEFT:
		megad.segavr_set_hmd_movement(1, moveAmount);
		break;
	case CTL_SEGAVR_HMD_YAW_RIGHT:
		megad.segavr_set_hmd_movement(1, -moveAmount);
		break;
	}
...
static int ctl_hmd_release(struct ctl& ctl, md& megad)
{
	switch (ctl.type)
	{
	case CTL_SEGAVR_HMD_PITCH_UP:
	case CTL_SEGAVR_HMD_PITCH_DOWN:
		megad.segavr_set_hmd_movement(0, 0.0f);
		break;
	case CTL_SEGAVR_HMD_YAW_LEFT:
	case CTL_SEGAVR_HMD_YAW_RIGHT:
		megad.segavr_set_hmd_movement(1, 0.0f);
		break;
	}
...
#endif
#ifdef WITH_GXZ80
	case md::Z80_CORE_GXZ80:
		msg = "GXZ80 core activated.";
		break;
#endif
	default:
...
{
	pd_do_screenshot(megad, "", false);
	return 1;
}

#ifdef WITH_SEGAVR
static int ctl_hmd_quadshot(struct ctl&, md& megad)
{
...
#ifdef WITH_SEGAVR
	{ CTL_SEGAVR_HMD_PITCH_UP, &segavr_hmd_pitch_up, ctl_hmd, ctl_hmd_release, DEF },
	{ CTL_SEGAVR_HMD_PITCH_DOWN, &segavr_hmd_pitch_down, ctl_hmd, ctl_hmd_release, DEF },
...
next_event:
	...
	case SDL_JOYAXISMOTION:
		...
		{
		case 0:
			analogThreshold = dgen_segavr_axis0_threshold;
			break;
		case 1:
			analogThreshold = dgen_segavr_axis1_threshold;
			break;
		case 2:
			analogThreshold = dgen_segavr_axis2_threshold;
			break;
		case 3:
			analogThreshold = dgen_segavr_axis3_threshold;
			break;
		case 4:
			analogThreshold = dgen_segavr_axis4_threshold;
			break;
		default:
			break;
		}
#endif
		if (event.jaxis.value <= -analogThreshold)
			i = 0;
		else if (event.jaxis.value >= analogThreshold)
			i = 1;
		else
			i = 2;
		...
		for (struct ctl* ctl = control; (ctl->rc != NULL); ++ctl) {
			...
			for (i = 0; (i != pi); ++i) {
				if ((uint32_t)(*ctl->rc)[RCBJ] == plist[i]) {
					...
					{
						ctl->hasAxisAmount = true;
						ctl->axisAmount = abs(event.jaxis.value) - analogThreshold;
					}
#endif
