...
#ifdef PREFER_EXECUTABLE_DIR
//hacked in to keep things out of users/etc.
char *get_app_dir(char *buf, size_t *size, const char *sub)
{
	HMODULE appMod = GetModuleHandle(NULL);
	if (appMod)
	{
		char tempPath[4096];
		GetModuleFileName(appMod, tempPath, 4096);
		size_t l = strlen(tempPath);
		while (l > 0)
		{
			if (tempPath[l] == '\\' || tempPath[l] == '/')
			{
				break;
			}
			--l;
...
char *dgen_userdir(char *buf, size_t *size)
{
	...
	size_t sz;
#ifdef PREFER_EXECUTABLE_DIR
	char *pDir = get_app_dir(buf, size, NULL);
	if (pDir)
	{
		return pDir;
	}
...
{
#ifdef PREFER_EXECUTABLE_DIR
	char *pDir = get_app_dir(buf, size, sub);
	if (pDir)
	{
		return pDir;
	}
